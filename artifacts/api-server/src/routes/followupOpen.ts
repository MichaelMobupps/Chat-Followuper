...
// CF-R1: URL construction goes through the config module (Bundle 1/2), which
// is prefix-aware and non-throwing, instead of July's appPublicUrl() — the
// destinations stay July's.
import { absoluteAppUrl, absoluteApiUrl } from "../lib/appConfig";
...
function dashboardFallback(): string {
  // CF-R1 synthesis: July moved the fallback destination to the Contacts page
  // (restored by this merge); the migration work owns how the URL is built.
  // absoluteAppUrl is prefix-aware and non-throwing — with no address
  // configured it yields the bare rooted path, which matches July's catch
  // branch; under a prefix it yields /chat/contacts.
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (row.channel === "linkedin") {
        ...
        // CF-R1: absoluteApiUrl keeps this link on the prefixed API mount when
        // BASE_PATH is set; at the default base it is byte-identical to the
        // July spelling (origin + "/api/followups/fallback/…").
        res.redirect(
          302,
          `${absoluteApiUrl(`/followups/fallback/${followupId}`)}?t=${t}`,
        );
        return;
      }
      res.redirect(302, dashboardFallback());
