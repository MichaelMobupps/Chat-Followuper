 ...
 *    environment, so setting BASE_PATH on the deployment could never reach the
 *    build. The pin is gone, so the value now flows from the environment — and
 *    the default here is what keeps an unset environment identical to the
 *    pinned "/" it replaces.
 *
 ...
 *
 * `normalizeBasePath` returns "/" for anything it rejects, so a hostile value
 * produces a build byte-identical to the unset one.
 */
...
if (
  ...
  // build would turn a bad env var into an outage — but it must not be silent,
  // or a rejected prefix looks exactly like a successful root build.
  console.warn(
