...
- **FUP3** (cross-process atomic claim) and **FUP5** (channel-aware unique index) remain — both need a schema migration (unique constraint / index); moved to the migration bucket below. The in-process `running` guard + FUP2/FUP4 fixes close the same-process double-send; FUP3 covers the standalone-cron co-run case.
...
- Remaining FE (FE5/FE10/FE11/FE12/FE15 + a11y lows) — Med batch, next.
